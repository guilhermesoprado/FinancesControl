...
public sealed class InvoiceAdjustmentServiceTests
{
    [Fact]
    public async Task AdjustAsync_ShouldApplyCreditAndReduceInvoiceTotal()
    {
        var nowUtc = new DateTime(2026, 4, 13, 18, 0, 0, DateTimeKind.Utc);
        var userId = Guid.NewGuid();
        var creditCard = CreditCard.Create(userId, "Cartao", "Visa", 3000m, 10, 18, null, nowUtc.AddDays(-5));
        var invoice = CreateInvoiceWithAmount(userId, creditCard.Id, 240m, nowUtc.AddDays(-1));
        ...
        var result = await service.AdjustAsync(new AdjustInvoiceInput(userId, invoice.Id, InvoiceAdjustmentType.Credit, 40m), CancellationToken.None);
    ...
    [Fact]
    public async Task AdjustAsync_ShouldRejectManualDecreaseBelowAlreadyPaidAmount()
    {
        var nowUtc = new DateTime(2026, 4, 13, 18, 0, 0, DateTimeKind.Utc);
        var userId = Guid.NewGuid();
        var account = FinancialAccount.Create(userId, "Conta", FinancialAccountType.BankAccount, 500m, null, null, nowUtc.AddDays(-10));
        var creditCard = CreditCard.Create(userId, "Cartao", "Visa", 3000m, 10, 18, null, nowUtc.AddDays(-5));
        var invoice = CreateInvoiceWithAmount(userId, creditCard.Id, 100m, nowUtc.AddDays(-1));
        invoice.ApplyPayment(account.Id, 70m, nowUtc);
        var service = new InvoiceService(
        ...
        var exception = await Assert.ThrowsAsync<AppValidationException>(() => service.AdjustAsync(
            new AdjustInvoiceInput(userId, invoice.Id, InvoiceAdjustmentType.ManualDecrease, 40m),
            CancellationToken.None));

        Assert.Equal("O ajuste nao pode reduzir a fatura para menos do que o valor ja pago.", exception.Message);
    }
